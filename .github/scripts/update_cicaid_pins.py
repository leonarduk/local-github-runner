...
def _pin_re(key: str) -> re.Pattern[str]:
    ...
    """
    pattern = r"^(" + re.escape(key) + r"=)(" + _PIN_VALUE + r")\r?$"
    return re.compile(pattern, re.MULTILINE)


def _read_text(path: Path) -> str:
    # No "r": it is the default, and ruff's UP015 fails a consumer repo whose
    # own lint covers .github/scripts/ (a real leonarduk/doodlebug run).
...
def latest_version(dep: str, token: str | None = None) -> str:
    ...
    if not tag.startswith("v"):
        msg = f"unexpected {dep} release tag {tag!r}; expected a v-prefixed tag"
        raise PinError(msg)
    return tag[1:]
